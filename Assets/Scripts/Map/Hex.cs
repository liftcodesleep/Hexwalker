...
public class Hex {
  ...
  public Hex(int q, int r) {
    ...
    this.Constructs = new List<Construct>();
  }
  ...
  private int CalculateElevation() {
    ...
    x = (float)column / (float)(Game.map.columns * scale);
    y = (float)row / (float)(Game.map.rows * scale);
  ...
  private int CalculateIslandElevationOffset() {
    int from_center = (Game.map.rows-1) / 2 - row;
  ...
  public float DistanceFrom(Hex b) {
    ...
    if (dQ > Game.map.columns / 2) {
      dQ = Game.map.columns - dQ;
    }
    int dS = Mathf.Abs(this.sum - b.sum);
    if (dS > Game.map.columns / 2) {
      dS = Mathf.Abs(dS - Game.map.columns);
    }
    return Mathf.Max(
      dQ,
